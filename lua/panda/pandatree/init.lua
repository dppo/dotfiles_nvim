...
local function get_tree_buffer()
  ...
  for _, buffer in ipairs(all_buffer) do
    if is_buf_match_name(buffer, buf_name) then
      tree_buffer = buffer
    end
  end
  return tree_buffer
...
local function load_local_config()
  ...
  for _, v in pairs(local_commands) do
...
local function close_tree_window(window)
...
local function wait_sync_tab_tree()
  wait_sync = true
end
...
local function prevent_other_buffers()
  if vim.fn.bufname("#") == buf_name and vim.fn.bufname("%") ~= buf_name and vim.fn.winnr("$") > 1 then
    local tree_window = get_cur_tab_tree_window()
    if tree_window == nil then
