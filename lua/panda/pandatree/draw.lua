...
local function draw_tree(buf, open_tree)
  ...
  -- 根目录
  local root_name = icon["root"]["symbol"] .. setting["root_name"] .. util.get_name(cwd)
  local lines = {
    util.handler_line_name(root_name, setting["win_width"] - 2, setting["space"]) .. "  "
  }
  local hl_color = {
    {
      {
  ...
  for k, v in pairs(tree) do
    ...
    local line_name = v.indent .. v.icon.symbol .. folder_name_space .. v.name
    table.insert(lines, util.handler_line_name(line_name, setting["win_width"] - 2, setting["space"]) .. status_icon)
    local line_color = {}
    local icon_color = {
